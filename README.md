...
**mybk-planner** is a read-only command-line tool for HCMUT (Đại học Bách Khoa TP.HCM) students that turns the myBK portal into a **course planner**: GPA tracker, transcript reader, CTĐT curriculum progress, timetable, exam schedule, and a next-semester course recommendation engine with **tuition estimates** — all from your terminal, in Vietnamese and English output.
...
- **Course planner** — `plan` combines curriculum + grades (+ the live open-class feed) and suggests a next-semester course list, prioritising retakes, filling each khối's gap up to its own remaining credits, and staying inside your per-semester credit budget.
...
- **Timeline** uses incremental credits between semesters, `ceil(remaining / max_tc)` semesters at your budget.
...
Run with no subcommand for a menu-driven session (one CAS login, then `1`–`8` plus `find`, `help`):
...
- **CAS**: myBK sits behind CAS 3.5.1 (`sso.hcmut.edu.vn/cas` for `/app`). A 403 during login usually means **rate-limited** — wait, don't spam.
