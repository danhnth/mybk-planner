...
**mybk-planner** là công cụ CLI read-only dành cho sinh viên Đại học Bách Khoa TP.HCM, giúp lập kế hoạch học tập ngay trên cổng myBK: theo dõi GPA, xem bảng điểm, tiến độ chương trình đào tạo (CTĐT), thời khóa biểu, lịch thi, cùng gợi ý môn học cho học kỳ tới kèm dự tính học phí. Tất cả ngay trên terminal.

Công cụ read-only: **tìm** lớp và **gợi ý** kế hoạch, còn việc đăng ký vẫn diễn ra trên cổng chính thức.
...
- **GPA & bảng điểm**: GPA tích lũy theo thang 10 và thang 4, lịch sử GPA từng học kỳ, phân bố điểm chữ, danh sách môn điểm D có thể cải thiện, và xử lý đúng trường hợp học lại đã đạt: môn từng rớt F nhưng sau đó đạt (D+) không còn bị tính là "chưa đạt".
- **Tiến độ CTĐT**: tiến độ tín chỉ theo từng khối kiến thức so với yêu cầu chương trình, tính đúng các khối có nhiều ràng buộc (khối BB + tổ hợp tự chọn + đồ án được cộng dồn, không gộp làm một).
- **Thời khóa biểu & lịch thi**: xem thời khóa biểu và lịch thi cho bất kỳ học kỳ nào.
- **Trình lập kế hoạch**: lệnh `plan` kết hợp CTĐT + bảng điểm (+ danh sách lớp mở nếu có đợt) để gợi ý môn học kỳ tới. Công cụ ưu tiên môn học lại, lấp đúng phần còn thiếu của từng khối, và không vượt hạn mức tín chỉ mỗi học kỳ.
- **Dự tính học phí**: `plan` in ra học phí ước tính theo thông báo học phí 2026-2027 chính thức: học phí trọn gói theo chương trình của bạn, tín chỉ vượt định mức 18 TC/HK tính theo đơn giá, và các mức giảm học phí (Bảng 1.2) khi đăng ký ít tín chỉ.
- **Phần read-only của đăng ký môn**: xem đợt đăng ký hiện tại, lớp mở, phiếu đăng ký của bạn, và đợt hoãn thi. Các endpoint ghi **cố tình không** được bao bọc.
...
- **Read-only và an toàn**: không bao giờ có wrapper `tao-phieu-dang-ky` / `huy-phieu-dang-ky`. Quy trình đăng ký của bạn không bị ảnh hưởng.
- **Vừa CLI vừa thư viện**: bảng đẹp trên terminal, JSON thô với `--json`, và các hàm thuần túy (`analysis`, `fees`) để tự viết script.
- **Tôn trọng quyền riêng tư**: thông tin đăng nhập lấy từ file `.env` được git-ignore, không bao giờ bị commit; công cụ chỉ dùng cho tài khoản của chính bạn.
...
cd mybk-planner
pip install -e ".[dev]"      # kèm pytest + ruff cho development
```

Với cả hai cách, bạn đều có lệnh `mybk-planner` (hoặc chạy `python -m mybk_planner.cli`).
...
Thứ tự ưu tiên đọc thông tin đăng nhập: **tham số CLI → biến môi trường → file `.env`**. Các tên cũ `MYBK_TEST_USERNAME`/`MYBK_TEST_PASSWORD`/`MYBK_TEST_MSSV` vẫn dùng được như dự phòng.
...
Mọi lệnh đều in bảng đẹp theo mặc định; thêm `--json` để lấy JSON thô cho script:
...
2. Lấp đúng phần còn thiếu của từng khối chưa đạt, khối thiếu nhiều xử lý trước, để một khối không "nuốt" hết hạn mức tín chỉ trong khi Tốt nghiệp vẫn trống.
3. Phần hạn mức còn dư đổ vào các môn của khối đã đạt, theo độ ưu tiên.
4. In bảng phân tích đầy đủ: diễn biến GPA, % hoàn thành, các khối còn thiếu, tình trạng điểm số, thời gian tốt nghiệp dự kiến, và dự tính học phí.
...
- Phần còn thiếu của khối là tổng các dòng yêu cầu riêng biệt. Feed lặp lại yêu cầu của mỗi nhóm trên mọi dòng môn học, và một khối có thể mang nhiều nhóm cộng dồn (ví dụ Chuyên ngành = khối BB + tổ hợp tự chọn + đồ án). Cách đọc "lấy dòng đầu tiên" sẽ tính thiếu chương trình.
- Ước thời gian còn lại dựa trên chênh lệch tín chỉ giữa các học kỳ: số học kỳ cần thêm là `ceil(còn lại / max_tc)` theo hạn mức của bạn.
- Học phí (theo thông báo 2026-2027): phí trọn gói theo chương trình, tín chỉ vượt định mức 18 TC/HK tính theo đơn giá, và các mức giảm ≤12/≤9/≤6 TC (15/30/45%) hiển thị khi kế hoạch của bạn đủ điều kiện.
...
- **Hậu tố `?null`** được thêm vào tham số đầu tiên của các GET (quirk chống cache của `/app/js/main.js`).
- **BOM UTF-8** (`\ufeff`) được loại bỏ khỏi response.
...
- **Read-only.** Các endpoint ghi của đăng ký và hoãn thi (`tao-phieu-dang-ky`, `huy-phieu-dang-ky`, `cap-nhat-…`) được phát hiện trong bundle API nhưng **cố tình không** được bao bọc.
...
Không. Đây là client read-only không chính thức do cộng đồng duy trì, dành cho cổng myBK.
...
Vì phần còn thiếu của một khối là tổng các dòng yêu cầu *riêng biệt*, và feed lặp lại giá trị yêu cầu trên mọi dòng môn học. Cộng dồn thô mọi môn tự chọn sẽ tính thừa.
